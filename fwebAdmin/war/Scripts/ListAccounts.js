...
fanikiwa.accountendpoint.listaccounts.iLoadAccounts = function() {
...
fanikiwa.accountendpoint.listaccounts.init = function(apiRoot) {
	...
	var callback = function() {
		if (--apisToLoad == 0) {
			...
			fanikiwa.accountendpoint.listaccounts.iLoadAccounts();
		}
	}
...
function populateAccounts(resp) {
	...
	if (!resp.code) {
		...
		for (var i = 0; i < resp.items.length; i++) {
			...
			if (resp.items[i].closed === true) {
				accountsTable += '<td>true</td>';
			} else {
				accountsTable += '<td>false</td>';
			}
