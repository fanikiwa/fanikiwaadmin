...
function PopulateDiaryprogramcontrolTable(resp) {
	...
	if (!resp.code) {
		...
		diaryprogramcontrolTable += "<th>Last Run</th>";
		diaryprogramcontrolTable += "<th>Next Run</th>";
		diaryprogramcontrolTable += "</tr>";
		diaryprogramcontrolTable += "</thead>";
		...
		for (var i = 0; i < resp.result.items.length; i++) {
			...
			diaryprogramcontrolTable += '<td>'
					+ formatDate(resp.result.items[i].lastRun) + '</td>';
			diaryprogramcontrolTable += '<td>'
					+ formatDate(resp.result.items[i].nextRun) + '</td>';
		}
